...
def orientation_set():
    for x in (-1, 1):
        for y in (-1, 1):
            for z in (-1, 1):
                yield lambda p, _x=x, _y=y, _z=z: Point(x=_x*p.x, y=_y * p.y, z=_z * p.z)
                yield lambda p, _x=x, _y=y, _z=z: Point(x=_x*p.x, y=_y * p.z, z=_z * p.y)

                yield lambda p, _x=x, _y=y, _z=z: Point(x=_x*p.y, y=_y * p.x, z=_z * p.z)
                yield lambda p, _x=x, _y=y, _z=z: Point(x=_x*p.y, y=_y * p.z, z=_z * p.x)

                yield lambda p, _x=x, _y=y, _z=z: Point(x=_x*p.z, y=_y * p.y, z=_z * p.x)
                yield lambda p, _x=x, _y=y, _z=z: Point(x=_x*p.z, y=_y * p.x, z=_z * p.y)
...
def realign_scanner(scanner: ScannerReadout, orientation: Orientation) -> ScannerReadout:
    return ScannerReadout(name=scanner.name,
                          beacons=reorient_to_default(scanner.beacons, orientation))
...
def find_unique_points(scanners: List[ScannerReadout]) -> Tuple[Set[Point], Dict[str, Frame]]:
    ...
    while to_align:
        ...
        newly_aligned = []
        print(f"Aligned {len(aligned_scanners)} of {len(scanners)} scanners")
        unaligned_beacons = []
        for working_scanner in to_align:
            ...
            for ref_scanner in check_this_time:
                try:
                    working_frame, working_orientation =\
                        find_reference_frame(ref_scanner, working_scanner, 12)
    ...
    unique_points = set(ref_scanner.beacons)
    for working_scanner in aligned_scanners[1:]:
        working_beacons =\
            [apply_reference_frame(
                beacon,
                ref_frames[working_scanner.name]) for beacon in working_scanner.beacons]
        for beacon in working_beacons:
            unique_points.add(beacon)
    return unique_points, ref_frames


def find_aligned_frames(lhs: ScannerReadout,
                        rhs: ScannerReadout,
                        threshold: int) -> Tuple[Frame, Frame, Orientation]:
    """
    We want to find a frame where the beacons settle into the same coordinates. Fortunately
    we know that both readouts have a number of shared points so there are a fixed of
    frames we can inspect.

    Since there are multiple common Beacons, there are multiple valid answers here - there
    is no "best", we just want one that works
    """
...
def find_reference_frame(reference: ScannerReadout,
                         other: ScannerReadout,
                         threshold: int) -> Tuple[Frame, Orientation]:
